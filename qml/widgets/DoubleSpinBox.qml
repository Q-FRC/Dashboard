...
Control {
  ...
  //!< Optional string to display before the value. See notes in the main comments above.
  property string suffix
  //!< Optional string to display after the value. See notes in the main comments above.
  ...
  readonly property real topValue: Math.max(from,
                                            to) //!< The effective maximum value
  readonly property real botValue: Math.min(from,
  ...
  readonly property QtObject regExpValidator: RegularExpressionValidator {
    regularExpression: control.doubleValidationRegEx()
  }

  // signals
  signal valueModified
  //!< Mimic SpinBox API (interactive change only, NOT emitted if \e value property is set directly).
  ...
  contentItem: SpinBox {
    ...
    from: -0x7FFFFFFF
    to: 0x7FFFFFFF // prevent interference with our real from/to values
  ...
  function setValue(newValue, noWrap, notModified) {
    ...
    if (value !== newValue) {
      ...
      if (spinBoxItem)
        spinBoxItem.value
            = 0 // reset this to prevent it from disabling the buttons or other weirdness
      return true
    }
    return false
  }
  ...
  function textFromValue(value, locale) {
    ...
    var text = value.toLocaleString(
          locale, (notation === DoubleValidator.StandardNotation ? 'f' : 'e'),
          Math.max(decimals, 0))
